...
public class MyAtoi {
    ...
    public void test() {
        ...
        Assert.assertEquals(12, myAtoi("12 hello world"));
        Assert.assertEquals(-12, myAtoi("-12"));
    }
    ...
    public int myAtoi(String str) {
        ...
        //1.跳过空字符串
        //2.读取第一个有效字符 判断是不是数字的asci 或是正负号
        while(index < length && (chars[index] == ' ' ||  chars[index] == '+' || chars[index] == '-')) {
            if(chars[index] == '-') { //找到一个符号就退出循环 +-234  这样的返回0
                index++;
                isPositive = false;
                break;
            }else if(chars[index] == '+') {
        ...
        while(index < length && chars[index] >= '0' && chars[index] <= '9') {
            ...
            if(Integer.MIN_VALUE / 10 > result || (Integer.MIN_VALUE / 10 == result && Integer.MIN_VALUE % 10 > num)) {
